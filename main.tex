...
\item
  A \textbf{strict clock}.
\item
  A birth-death tree prior.
\item
  A normally distributed root calibration with standard deviation of 5 and mean of 50.
\item
  Leave the run length at 10 million steps and the sampling frequency as 1K so that your results are comparable to those provided.
\end{itemize}

Importantly, this XML file has been modified directly such that the starting tree has been provided and it remains fixed throughout the run. Continue in the following box if you wish to learn how to modify the XML, or proceed to the next section to assess model adequacy. 
...
Newick refers to a format to express the tree structure. Replace the dummy newick tree for the correct tree, which you will find in \lstinline!chron.tre! in the \lstinline!data! folder. Also modify the names of the tree and the taxa: the name of your tree appears after \lstinline!"Tree.t:! elsewhere in the XML, and the name of the taxa is an \lstinline!@! followed by your data \lstinline!id! in the line immediately above the alignment.
...
Begin by opening R. By typing the following, you will set the working directory to the scripts folder, and then source all the functions inside it.
...
The next section will set the directory to \lstinline!precooked_runs!, and run the
function \lstinline!adeq()!. The arguments for \lstinline!adeq()! are the
...
performed. You can use different paths for the posterior data if you ran your own
...
clock and substitution model adequacy. Explore the names of the contents of \lstinline!clock_adequacy_example! and make sure you understand each of them.
...
The following section describes the steps for assessing model adequacy in more detail. \clearpage
...
\lstinline!precooked_runs/results.Rdata! and \textbf{not} those from the previous steps. If you want to use your results you need to replace \lstinline!assessment_provided! for the name of your results (\lstinline!clock_adequacy_example!). In this section we will graph different components
...
data is not so close to 1. The reason for this is in part that we used an overly simple model of rate variation across lineages. As described in section 3, the data were generated under an uncorrelated lognormal clock model, while the analyses in BEAST 2 was made using a strict clock. Furthermore, these data were generated under very high overall rates, such that they have a very large amount of variation, and possibly contain substantial
substitutional saturation. For comparison, you might want to repeat all
of the analyses, or explore the provided results of the posterior for the second data set
provided, \lstinline!al.2.nex!. This data set evolved through the same
tree but with lower rates of molecular evolution. The XML file for these data are called \lstinline!sim.2.xml! in the \lstinline!xml! folder, and the results from the BEAST 2 run are found in the \lstinline!precooked_runs! folder.
...
\item
  The test statistic used is critical to the assessment of model adequacy. The test statistic can define what aspect of a model is being assessed. Most test statistics are not complete descriptions of the model, so it is convenient to explore a model using several test statistics where possible.
\item
