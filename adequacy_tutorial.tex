...
\begin{lstlisting}[language=R]
    ...
    clock_adequacy_example <- adeq(trees.file = "sim.trees", log.file ="sim.log", empdat.file = "../data/al.nex", Nsim = 100)
...
Also investigate the \emph{A} index:

\begin{lstlisting}[language=R]
   assessment_provided[4]
\end{lstlisting}

This index provides an additional piece of information: it is the proportion of branches in the tree for which the branch-wise posterior predictive \emph{P}-values are inside the central 95 percent of the distribution. The rates and times models can be considered adequate when the \emph{A} index is high.

\begin{figure}
    \centering
    \includegraphics[width=0.800000\textwidth]{figures/Figure_3_branchwise_p.jpg}
...
\begin{lstlisting}[language=R]
    plot(tr, edge.col = rainbow(length(assessment_provided$branch_length_deviation), start = 2/6, end = 4/6)[rank(assessment_provided$branch_length_deviation)], edge.width = 6, cex = 1.5)
    edgelabels(round(assessment_provided$branch_length_deviation, 2), bg = "white", cex = 1.5, frame = "none")
\end{lstlisting}

\begin{figure}
    \centering
    \includegraphics[width=0.800000\textwidth]{figures/Figure_4_branchwise_dev.jpg}
